...
<div class="row">
    <div class="col-12">
        ...
        <div class="detail-section">
            ...
            @php
                // Cek apakah user bisa download (admin selalu bisa, pemohon hanya jika pengujian selesai)
                $canDownload = auth()->user()->isAdmin() || ($permohonan->pengujian_selesai ?? false);
            @endphp
            ...
            @if($validasi && $validasi->file_kaji_ulang_multiple && count($validasi->file_kaji_ulang_multiple) > 0)
                @foreach($validasi->file_kaji_ulang_multiple as $file)
                    @php
                        ...
                        $encodedPath = urlencode($file);
                        $fileUrl = route('file.show', ['path' => $encodedPath]);
