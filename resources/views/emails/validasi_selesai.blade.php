...
<body>
    <div class="container">
        <div class="header">
            <h1>Validasi Selesai</h1>
            <div class="sub">Laboratorium Penguji Mutu Alsintan UPTD BMSPP</div>
        </div>
        <div class="body">
            <p>Yth. {{ $permohonan->nama_pemohon ?? 'Pemohon' }},</p>
            <div class="highlight">
                <p style="margin: 0;">
                    Permohonan Anda <strong>{{ $permohonan->nomor_surat_permohonan ?? 'PMH-'.str_pad($permohonan->id, 6, '0', STR_PAD_LEFT) }}</strong>
                    telah <strong>divalidasi</strong> oleh admin.
                </p>
                <p style="margin: 5px 0 0 0; font-size: 14px;">
                    <strong>Jenis Alsintan:</strong> {{ $permohonan->jenis_alsintan }}
                </p>
                <p style="margin: 5px 0 0 0; font-size: 14px;">
                    <strong>Tanggal Validasi:</strong> {{ now()->format('d M Y H:i') }}
                </p>
            </div>
            <p>Silakan login ke sistem untuk melihat detail validasi dan melanjutkan ke tahap pengujian.</p>
